...
audio.addEventListener("timeupdate",updateProgress);$(".progress-container").click(function(e){setProgress(e)})});function setProgress(e){const width=$(".progress-container").width();const clickX=e.offsetX;const duration=audio.duration;audio.currentTime=(clickX/width)*duration}
function convertToTime(duration){if(!isNaN(duration)){let minutes=parseInt(duration/60,10);let seconds=parseInt(duration%60);if(minutes<10){minutes="0"+minutes}
if(seconds<10){seconds="0"+seconds}
return `${minutes}:${seconds}`}else{return"00:00"}}
...
function prevSong(){pauseSong();audio.currentTime=0;$(".progress-time").html("00:00");const isPlaying=$(".album-art").hasClass("active");if(!isPlaying){$(".album-art").addClass("active");$(".player-track").addClass("active")}
playSong()}